...
const ManageProducts = () => {
  ...
  const loadProducts = async () => {
    const result = await queryProducts(pageable.pageNumber, pageable.pageSize);
  ...
  return (
    <>
      ...
      {/* Add new product */}
      <Popup isOpen={addingProduct} onClose={() => setAddingProduct(false)}>
        <EditProduct
      ...
      <ProductList
        ...
        deleteProduct={(product: Product) => handleDeleteProduct(product)}
        editDiscounts={(product: Product) => setDiscountEditing(product)}
      />
    </>
  );
};
